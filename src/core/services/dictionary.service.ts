...
export class DictionaryService {
  private mappingsCache: Map<string, string> | null = null;
   ...
   */
  async addEntry(source: string, target: string): Promise<DictionaryEntry> {
    const now = new Date();
  ...
  async listEntries(): Promise<DictionaryEntry[]> {
    return this.storage.listDictionaryEntries();
  }
  ...
  async correctText(text: string): Promise<string> {
    ...
    if (!this.mappingsCache) {
      this.mappingsCache = await this.storage.getAllDictionaryMappings();
    }

    if (this.mappingsCache.size === 0) {
      return text;
    }

    // Sort sources by length (longest first)
    const sortedSources = Array.from(this.mappingsCache.keys()).sort(
    ...
    for (const source of sortedSources) {
      const target = this.mappingsCache.get(source)!;
